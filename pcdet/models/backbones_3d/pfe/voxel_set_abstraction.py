...
def sample_points_with_individual_roi(rois, points, sample_extra_width, enrich_method=None):
    """
    Args:
        rois: (M, 7 + C)
        points: (N, 3)
    ...
    Returns:
        sampled_points: (M, N_out, 3)
    """
    from ....ops.roipoint_pool3d import roipoint_pool3d_utils

    num_points = point.shape[0]
    num_rois = rois.shape[0]
    sample_extra_width = sample_extra_width if len(sample_extra_width) == 3 else sample_extra_width[0] * [1, 1, 1]
    extend_roi = box_utils.enlarge_box3d(rois, extra_width=sample_extra_width).unsqueeze(dim=0)

    point_assignment = roipoint_pool3d_utils.points_in_boxes_gpu(points.unsqueeze(dim=0), extend_gt_boxes[:, :, 0:7]) # [B,N,M]
    point_mask = point_assignment.squeeze(0).permute(1,0) # [M, N]
    point_in_rois_stack = point_mask.sum(dim=1)
    empty_flag = point_in_roi_stack == 0
    sampled_points = torch.cat([(points[:1] if empty_flag[i] else points[point_mask[i,:],:]) for i in range(M)], dim=0)



    transformed_points = sampled_points[fg_flag] - rois[:, 0:3]
    transformed_points = common_utils.rotate_points_along_z(
        transformed_points.view(-1, 1, 3), -gt_box_of_fg_points[:, 6]).view(-1, 3)

    return sampled_points, point_mask


class VoxelSetAbstraction_v1(nn.Module):
    def __init__(self, model_cfg, voxel_size, point_cloud_range, num_bev_features=None,
                 num_rawpoint_features=None, **kwargs):
        super().__init__()
        self.model_cfg = model_cfg
    ...
    def sectorized_proposal_centric_sampling(self, roi_boxes, points):
        ...
        sampled_points, _ = sample_points_with_individual_roi(
            rois=roi_boxes, points=points,
            sample_extra_width=self.model_cfg.SPC_SAMPLING.SAMPLE_EXTRA_WIDTH,
            num_max_points_of_part=self.model_cfg.SPC_SAMPLING.get('NUM_POINTS_OF_EACH_SAMPLE_PART', 200000)
        )
        sampled_points = sector_fps(
            points=sampled_points, num_sampled_points=self.model_cfg.NUM_KEYPOINTS,

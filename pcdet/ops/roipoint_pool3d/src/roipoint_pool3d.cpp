...
void assignpointsLauncher(int batch_size, int pts_num, int boxes_num,
                       const float *xyz, const float *boxes3d, int *pts_assign);

int assign_points_to_box_gpu(at::Tensor xyz, at::Tensor boxes3d, at::Tensor pts_assign_tensor){
    // params xyz: (B, N, 3)
    // params boxes3d: (B, M, 7)
    // params pooled_empty_flag: (B, N, M)
    CHECK_INPUT(xyz);
    CHECK_INPUT(boxes3d);
    CHECK_INPUT(pts_assign_tensor);
    ...
    const float *xyz_data = xyz.data<float>();
    const float *boxes3d_data = boxes3d.data<float>();
    int *pts_assign_data = pts_assign_tensor.data<int>();

    assignpointsLauncher(batch_size, pts_num, boxes_num,
                       xyz_data, boxes3d_data, pts_assign_data);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("assign_points", &assign_points_to_box_gpu, "assigne points to boxes3d (CUDA)");
}

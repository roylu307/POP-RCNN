...
def points_in_boxes_gpu(points, boxes):
    """
    :param points: (B, N, 3)
    :param boxes: (B, M, 7), num_valid_boxes <= T
    :return box_idxs_of_pts: (B, N, M), default background = -1
    """
    assert boxes.shape[0] == points.shape[0]
    assert boxes.shape[2] == 7 and points.shape[2] == 3
    batch_size, num_points, _ = points.shape
    _, num_box, _ = boxes.shape

    pts_assign = points.new_zeros((batch_size, num_points, num_box), dtype=torch.int).fill_(-1)
    roipoint_pool3d_cuda.assign_points(points.contiguous(), boxes.contiguous(), pts_assign)

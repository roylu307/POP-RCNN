...
def build_local_aggregation_module(input_channels, config):
    ...
    if local_aggregation_name == 'VectorPoolAggregationModuleROIMSG':
        cur_layer = VectorPoolAggregationModuleROIMSG(input_channels=input_channels, config=config)
...
class VectorPoolAggregationModule(nn.Module):
    ...
    def forward(self, xyz, xyz_batch_cnt, new_xyz, new_xyz_batch_cnt, features, **kwargs):
        ...
        :param xyz_roi_cnt: (num_rois), [R1, R2, ...]
        :param new_xyz: (M1 + M2 ..., 3)
        :param new_xyz_batch_cnt: (batch_size), [M1, M2, ...]
        :param new_xyz_roi_cnt: (num_rois), [R1, R2, ...]
        ...
        features = features if C==1 else features.view(N, -1, self.num_reduced_channels).sum(dim=1)

        for bs_idx in range(xyz_batch_cnt.shape[0]):
...
class VectorPoolAggregationModuleROIMSG(nn.Module):

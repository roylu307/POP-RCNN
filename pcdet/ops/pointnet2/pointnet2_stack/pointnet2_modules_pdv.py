...
from . import pointnet2_utils_pdv
...
class StackSAModuleMSG(nn.Module):
    ...
    def __init__(self, *, radii: List[float], nsamples: List[int], mlps: List[List[int]],
        ...
        for i in range(len(radii)):
            radius = radii[i]
            nsample = nsamples[i]
            self.groupers.append(pointnet2_utils_pdv.QueryAndGroup(radius, nsample, use_xyz=use_xyz, use_density=use_density))
            mlp_spec = mlps[i]
            if use_xyz:
                mlp_spec[0] += 3
            if use_density:
...
class StackPointnetFPModule(nn.Module):
    ...
    def forward(self, unknown, unknown_batch_cnt, known, known_batch_cnt, unknown_feats=None, known_feats=None):
        ...
        """
        dist, idx = pointnet2_utils_pdv.three_nn(unknown, unknown_batch_cnt, known, known_batch_cnt)
        dist_recip = 1.0 / (dist + 1e-8)
        norm = torch.sum(dist_recip, dim=-1, keepdim=True)
        weight = dist_recip / norm

        interpolated_feats = pointnet2_utils_pdv.three_interpolate(known_feats, idx, weight)
